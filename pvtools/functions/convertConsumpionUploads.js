...
const convertConsumptionCSV = (csvData, year) => {
    ...
    if (!(filteredEmpty.length == 365*24) && !(filteredEmpty.length == 366*24) && !(filteredEmpty.length == 48)) throw new Error('Length of your data must be 365/366 days * 24 hours')

    const importYear = parseInt(filteredEmpty[0].Datetime.slice(0,4))
    if (importYear !== year) throw new Error('Import year is wrong. You want to import year '+importYear+' but you set year '+year+'!')
    ...
    dayTimes.forEach(daytime => {
        ...
        if (!found) throw new Error('Following timeslot not found:' + daytime)
    })
    filteredEmpty.forEach(val => {
        if (!dayTimes.includes(val.Datetime))  throw new Error('at least one DateTime is wrong: ' + val + val.Datetime)
    })
